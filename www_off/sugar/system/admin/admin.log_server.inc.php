...
if($sugar->security_admin<2)$js->alert_die("No acces");
if(isset($_POST['limit']))$limit=$_POST['limit'];
else $limit=100;
$sugar->admin_menu($system,$box);
$sugar->label="adm";
$user_ok=0;
$sys_ok=0;
$sub_sys_ok=0;
...
$form->open("index.php?system=admin/admin.log_server","POST");
	
	echo "Utilisateur : ";
	$form->select_in('user',1);
	$user_tmp=db_read("select distinct id_tech from admin_log_server;");
		$form->select_option('Tous',0,$user_ok);
		while($user = $user_tmp->fetch())$form->select_option($sugar->show_name_user($user->id_tech),$user->id_tech,$user_ok);
	$form->select_out();
	echo "System : ";
	if(isset($user_ok))
		{
...
echo "<center><table border=1 cellspacing=0 width=80%><tr bgcolor=white><th>Id</th><th>Date</th><th>Tech</th><th>System</th><th>Sub_system</th><th>Remarque</th></tr>";
//echo $rqt;
$log_tmp=db_read($rqt);
while($log = $log_tmp->fetch())
	...
	echo "<tr bgcolor=$color><td>$log->id</td><td>".date('d/m/y - H:i',$log->dt)."</td><td>".$sugar->show_name_user($log->id_tech)."</td><td>$log->system</td><td>$log->sub_system</td><td>$log->remarque</td></tr>";
	}
echo "</table></center>";
