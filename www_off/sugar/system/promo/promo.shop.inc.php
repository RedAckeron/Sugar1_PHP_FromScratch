...
######################################################################################################################
	case 'lobby':
		...
		$sugar->label="prm";
		/*
		$box->box_rond('','','200','23','yellow','black','','',100,"Ajouter une promo","index.php?system=promo&sub_system=add_prm",'');
		//$box->box_rond('','',200,'23','yellow','black','','',100,"Ajouter un screen","index.php?system=prm&sub_system=add_screen",'');
		$box->box_rond('','','1298','23','black','black','','',100,"<marquee><font color=lightblue size=3>Promo appliquable a toutes les implantations<font color=black>______________ <font color=lightgreen>Promo appliquable uniquement a cette implantation<font color=black>______________<font color=#b68bfc>Panneau publicitaire</font></marquee>","",'');		
		*/
		...
		$box->angle_out('');
		/*
		$cmmd_tmp=db_read("select * from cmd where status = 'open' and type='prm' and id_shop=0 order by id_shop,titre;");
		while($cmmd = $cmmd_tmp->fetch())
			{
			$cmd->load($cmmd->id);
			$cmd->show_line_promo_v2($box,$js,$sugar);
			}
		*/
		$cmmd_tmp=db_read("select * from cmd where status = 'open' and type='prm' and id_shop=$sugar->shop_id order by id_shop,titre;");
		while($cmmd = $cmmd_tmp->fetch())
			{
			$cmd->load($cmmd->id);
			$cmd->show_line_promo_v2($box,$js,$sugar);
...
######################################################################################################################
    case 'add_prm':
		{
		$chk_prm=db_single_read("select count(*) as cnt from cmd where type='prm' and id_shop = '$sugar->shop_id' and status !='hidden';");
		if($chk_prm->cnt<10)
			{
			$dt_in=time();
			$dt_out=0;
			db_write("insert into cmd (id_customer,type,titre,tech_in,dt_in,dt_out,status,id_shop)values('$sugar->shop_id','prm','Promo','$sugar->id','$dt_in','$dt_out','open','$sugar->shop_id');");
			...
			echo "<meta http-equiv='refresh' content='0; url=index.php?system=promo&sub_system=edit_promo&id_prm=$chk_prm->id'/>";
			}
		else $js->alert_redirect("Vous ne pouvez introduire que 10 promo maximum par implantation","index.php?system=promo",0);
		};break;
######################################################################################################################
    case 'add_screen':
		{
		$dt_in=time();
		$dt_out=0;
		db_write("insert into cmd (id_customer,type,titre,tech_in,dt_in,dt_out,status,id_shop)values('$sugar->shop_id','prm','Screen','$sugar->id','$dt_in','$dt_out','SCREEN','0');");
		//on recupere l id du ticket qu on viens de creez
		$chk_prm=db_single_read("select * from cmd order by id desc limit 1");
		db_write("insert into promo(id_odp,bground,img0,img1,img2,img3,img4,img5,img6)values('$chk_prm->id','dotted.gif','no_img.gif','no_img.gif','no_img.gif','no_img.gif','no_img.gif','no_img.gif','no_img.gif');");
		echo "<meta http-equiv='refresh' content='0; url=index.php?system=prm&sub_system=edit_screen&id_prm=$chk_prm->id'/>";
		};break;
######################################################################################################################
	case 'edit_promo':
		{
		...
		echo "$id_promo->id_odp";
		//si la promo n est pas un screen on affiche la liste des items
		if($cmd->status!='SCREEN')
			{
			//affichage de la box info 
			$box->angle_in('20','40','1150','380','white','red','','',100,"","",'');
			echo "<iframe frameborder=0 src='index.php?system=promo&sub_system=edit_info&id_promo=$id_promo->id&no_interface' width=100% height=100%></iframe>";
			$box->angle_out('');

			//affichage de la liste des item de la commande 
			$box->angle_in('20','433',1458,380,'white','red','','',100,"","",'');
			echo "<iframe frameborder=0 src='index.php?system=cmd/item.iframe&sub_system=show_item&id_cmd=$id_prm&no_interface' width=100% height=100%></iframe>";
			$box->angle_out('');

			//afficher les boutons
			$box->empty_in('1235','20',252,340,'','','','',100,"",'','');
				//imprimer flyer
				echo "<a href='index.php?system=promo&sub_system=print_promo&id_prm=$id_promo->id_odp&no_interface' target = _blank>";
				$box->angle('','',250,25,'lightgreen','black','','',100,"Imprimer Promo Flyer",'','');//imprimer reception
				echo "</a>";
				$box->angle('','',250,25,'','','','',100,"","",'');
				//imprimer promo item
				echo "<a href='index.php?system=odp&sub_system=print_odp_customer&id_odp=$id_promo->id_odp&no_interface' target = _blank>";
				$box->angle('','',250,25,'lightgreen','black','','',100,"Imprimer promo item","",'');//imprimer reception
				echo "</a>";
				$box->angle('','',250,25,'','','','',100,"","",'');
				//dupliquer promo sur une odp
				$box->angle('','',250,25,'lightgreen','black','','',100,"Dupliquer promo sur offre de prix","index.php?system=promo&sub_system=duplicate_promo_to_odp&id_odp=$id_promo->id_odp&no_interface",'');//imprimer reception
				$box->angle('','',250,25,'','','','',100,"","",'');
				//effacer promo
				if($sugar->security_prm>1)
					{
					$box->angle('','',250,25,'#CC5BF4','black','','',100,"Detruire promo","index.php?system=promo&sub_system=destroy_promo&id_odp=$id_promo->id_odp",'');
					$box->angle('','',250,25,'','','','',100,"","",'');
					}
				
				if(($cmd->shop_id!=0)and($sugar->security_prm>1))$box->angle('','',250,25,'lightgreen','black','','',100,"Passer en Promo Groupe","index.php?system=odp&sub_system=duplicate_odp&id_odp=$cmd->id&no_interface",'');//imprimer reception
				else $box->angle('','',250,25,'lightgreen','black','','',100,"Passer en Promo Shop","index.php?system=odp&sub_system=duplicate_odp&id_odp=$cmd->id&no_interface",'');//imprimer reception
...
######################################################################################################################
	case 'update_screen':
		{
		$img=$_GET['img'];
		db_write("update promo set bground_fhd='$img' where id_odp=$id_prm");
		echo"<meta http-equiv='refresh' content='0; url=index.php?system=promo&sub_system=edit_screen&id_prm=$id_prm'/>";
		};break;
######################################################################################################################
	case 'edit_fiche':
		{
